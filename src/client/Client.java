...
public class Client extends StaticScreenGame {
	...
	public Client() {
		...
		for (int x = 0; x < netPlayers.size(); x++) {
			players.add(new Player("player", new Vector2D(100, 100), netPlayers
					.elementAt(x), control));
		...
		thisPlayer = players.elementAt(0);
		// player1 = new Player("player", new Vector2D(100,100), netPlayer1,
		// control);
	...
	public void update(long deltaMs) {
		...
		if (mouse.isLeftButtonPressed()) {
			Point pointy = mouse.getLocation();
			System.out.println("Weapon fire keypress" + pointy);
			thisPlayer.fireWeapon(pointy);
		}
	}

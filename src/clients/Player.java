...
public class Player {
	...
	public void move(int d) {
		...
		switch (moveState) {
		case LEFT:
			move = new Action(playerId, Action.CHANGE_VELOCITY, new Vector2D(
					-DELTA_V, 0));
			tcp.sendSocket(prot.encodeAction(move));
			break;
		case RIGHT:
			move = new Action(playerId, Action.CHANGE_VELOCITY, new Vector2D(DELTA_V,
					0));
			tcp.sendSocket(prot.encodeAction(move));
			break;
		case UP:
			move = new Action(playerId, Action.CHANGE_VELOCITY, new Vector2D(0,
					-DELTA_V));
			tcp.sendSocket(prot.encodeAction(move));
			break;
		case DOWN:
	...
	public void move(int x, int y) {
		...
		Action move;
		move = new Action(playerId, Action.CHANGE_VELOCITY, new Vector2D(DELTA_V * x,
				DELTA_V * y));
		tcp.sendSocket(prot.encodeAction(move));
		if (x == 0 && y == 0)
			moveState = HALT;
